...
with tab2:
    ...
    # Función para graficar un árbol binomial con mejor diseño para muchos nodos
def plot_binomial_tree(values, title, ax):
    ...
    for i in range(values.shape[0]):
        for j in range(i + 1):
            ...
            labels[node] = f"{values[i, j]:.2f}"  # Mostrar valores con 2 decimales (reducir precisión para ahorrar espacio)
            if i > 0:
...
with col1:
    fig1, ax1 = plt.subplots(figsize=(8, 6))  # Aumentar el tamaño de la figura
