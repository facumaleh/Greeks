...
with tab1:
    ...
    - **Segunda Derivada (\( f''(x) \)):**
      ...
      f''(x) = %s
      \]
    """ % (sp.latex(f_prime), sp.latex(f_double_prime)))
    ...
    - **Valor de \( f''(x_0) \):**
      ...
      f''(x_0) = %s
      \]
    """ % (sp.latex(f.subs(x, x0)), sp.latex(f_prime.subs(x, x0)), sp.latex(f_double_prime.subs(x, x0)))))
    ...
    - **Polinomio de Taylor de Grado 2 (\( T_2(x) \)):**
      ...
      Sustituyendo los valores:
      \[
      T_2(x) = %s + \frac{1}{2} \cdot %s \cdot (x - %s)^2
      \]
    """ % (sp.latex(f.subs(x, x0)), sp.latex(f_prime.subs(x, x0)), x0, sp.latex(taylor_1), sp.latex(f_double_prime.subs(x, x0)), x0))
    except Exception as e:
